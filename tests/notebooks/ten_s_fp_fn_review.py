...
for subject_id, data in artifact_data.items():
    ...
    for i, artifact_clip in enumerate(data):
        ...
        # Organize FOOOF
        fit_curves = {"spectra": spectra,
                      "frequencies": freqs,
                      "curves": np.zeros(shape=spectra.shape),
                      "r_squared": np.zeros(shape=spectra.shape[CHANNEL_AXIS]),
                      "mae": np.zeros(shape=spectra.shape[CHANNEL_AXIS]),
                      "mse": np.zeros(shape=spectra.shape[CHANNEL_AXIS]),
                      "rmse": np.zeros(shape=spectra.shape[CHANNEL_AXIS]),
                      "curve_offset": np.zeros(shape=spectra.shape[CHANNEL_AXIS]),
                      "curve_exp": np.zeros(shape=spectra.shape[CHANNEL_AXIS]),
                      "peak_freq": np.zeros(shape=spectra.shape[CHANNEL_AXIS]),
                      "peak_height": np.zeros(shape=spectra.shape[CHANNEL_AXIS]),
                      "peak_width": np.zeros(shape=spectra.shape[CHANNEL_AXIS]),
                      }
        for channel in range(spectra.shape[CHANNEL_AXIS]):
            fo = fg.get_fooof(channel)
            fit_curves["curves"][:, channel] = c_data = fo._ap_fit

            r_val = np.corrcoef(spectra[:, channel], c_data)
            r_squared = r_val[0][1] ** 2
            mae, mse, rmse = calculate_mean_errors(spectra[:, channel], c_data)

            fit_curves["r_squared"][channel] = r_squared
            fit_curves["mae"][channel] = mae
            fit_curves["mse"][channel] = mse
            fit_curves["rmse"][channel] = rmse
            fit_curves["curve_offset"][channel] = fo.aperiodic_params_[0]
            fit_curves["curve_exp"][channel] = fo.aperiodic_params_[1]
            if len(fo.peak_params_) > 0:
                fit_curves["peak_freq"][channel] = fo.peak_params_[0][0]
                fit_curves["peak_height"][channel] = fo.peak_params_[0][1]
                fit_curves["peak_width"][channel] = fo.peak_params_[0][2]
            else:
                print("No peak")
        ...
        artifact_metric = {
            ...
            "curve_offset": fit_curves["curve_offset"],
            "curve_exp": fit_curves["curve_exp"],
            "peak_freq": fit_curves["peak_freq"],
            "peak_height": fit_curves["peak_height"],
            "peak_width": fit_curves["peak_width"],
            "reviews": reviews,
        }
        ...
        ag_metrics["curve_offset"] += list(fit_curves["curve_offset"])
        ag_metrics["curve_exp"] += list(fit_curves["curve_exp"])
        ag_metrics["peak_freq"] += list(fit_curves["peak_freq"])
        ag_metrics["peak_height"] += list(fit_curves["peak_height"])
        ag_metrics["peak_width"] += list(fit_curves["peak_width"])
        for reviewer, channels in reviews.items():
            good_channels = np.zeros((spectra.shape[CHANNEL_AXIS],))
...
flat_metrics = []
for name, clips in artifact_metrics.items():
    for clip, metrics_ in enumerate(clips):
        for channel in range(0, len(metrics_["r_squared"])):
            single_metrics = {
                "subject": name,
                "clip": clip,
                "channel": channel,
                "raw_data": metrics_["raw_data"][:, channel],
                "spectra": metrics_["fit_curves"]["spectra"][:, channel],
                "frequencies": metrics_["fit_curves"]["frequencies"],
                "r_squared": metrics_["r_squared"][channel],
                "normal_entropy": metrics_["normal_entropy"][channel],
                "mae": metrics_["mae"][channel],
                "mse": metrics_["mse"][channel],
                "rmse": metrics_["rmse"][channel],
                "curve_offset": metrics_["curve_offset"][channel],
                "curve_exp": metrics_["curve_exp"][channel],
            }

            flat_metrics.append(single_metrics)

#%%
svms = [None] * shuffles
for t_set in range(0, shuffles):
...
for index, id in enumerate(fp_df["id"]):
    fp_metrics[index] = fm = flat_metrics[id]
    c_label = artifact_data[fm["subject"]][fm["clip"]]["channel_labels"][fm["channel"]]
    fp_labels[index] = f"{fm['subject']} Clip {fm['clip'] + 1} Channel {fm['channel'] + 1}: {c_label}"

fn_metrics = [None] * len(fn_df)
fn_labels = [""] * len(fn_df)
for index, id in enumerate(fn_df["id"]):
    fn_metrics[index] = fm = flat_metrics[id]
    c_label = artifact_data[fm["subject"]][fm["clip"]]["channel_labels"][fm["channel"]]
    fn_labels[index] = f"{fm['subject']} Clip {fm['clip'] + 1} Channel {fm['channel'] + 1}: {c_label}"

fp_raw = np.stack([m["raw_data"] for m in fp_metrics], axis=1)
fp_spectra = np.stack([m["spectra"] for m in fp_metrics], axis=1)

fn_raw = np.stack([m["raw_data"] for m in fn_metrics], axis=1)
...
ts_group.figure.write_html(OUT_DIR.joinpath("Performance_Plot_Peaks.html").as_posix())

#%%
# Plot False Positives
fp_fig = Figure()
fp_fig.update_layout(title="False Positives Peaks", margin=dict(t=60))
...
fp_group["spectra"].build(x=flat_metrics[0]["frequencies"], y=fp_spectra, labels=fp_labels)
...
fp_fig.show()
fp_fig.write_html(OUT_DIR.joinpath("FP_Plot_Peaks.html").as_posix())

# Plot False Negatives
fn_fig = Figure()
fn_fig.update_layout(title="False Negatives Peaks", margin=dict(t=60))
...
fn_sp_plot = SpectraPlot(subplot=fn_sub[0][2], x=flat_metrics[0]["frequencies"], y=fn_spectra, labels=fn_labels)
...
fn_fig.show()
fn_fig.write_html(OUT_DIR.joinpath("FN_Plot_Peaks.html").as_posix())
