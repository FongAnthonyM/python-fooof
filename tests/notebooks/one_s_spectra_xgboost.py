...
PLOT_NAME = "One_Second_Spectra_XGBoost"
TIME_AXIS = 0
CHANNEL_AXIS = 1
LOWER_FREQUENCY = 1
UPPER_FREQUENCY = 250
...
bin_reviews = {reviewer["name"]: deque() for reviewer in artifact_info}
bin_reviews |= {"Reviewer Intersection": deque(), "Reviewer Union": deque()}
ag_metrics = []

artifact_metrics = {}
flat_data = {"labels": deque(), "reviews": ag_reviews, "data": deque()}
bin_data = {
    "labels": deque(),
    "reviews": bin_reviews,
    "data": deque(),
...
# Bin Data
for i, channel in enumerate(flat_data["data"]):
    sample_window = int(window_size * SAMPLE_RATE)
    n_bins = len(channel) // sample_window
    for bin_ in range(n_bins):
        bin_c = channel[bin_*sample_window: (bin_ + 1) * sample_window]
        bin_data["data"].append(bin_c)
        bin_data["labels"].append(f"{flat_data['labels'][i]}: Bin{bin_}")
        for reviewer, channels in flat_data["reviews"].items():
            bin_data["reviews"][reviewer].append(channels[i])

#%%
# Create Metrics
for bin_c in bin_data["data"]:
    ...
    # Fitting
    fo.fit(freqs, spectra)

    spectra = np.log10(spectra)
    bin_data["spectra"].append(spectra)
    bin_data["frequencies"].append(freqs)

    ag_metrics.append(spectra)

# Load Data into Pandas Data Frame
review_dataframe = pd.DataFrame.from_dict(bin_reviews)
...
#%%
def create_xgboost(metrics_df, review_df, mas, rs):
    # Create Train and Test Datasets
    data_split = train_test_split(metrics_df, review_df, random_state=rs, stratify=review_df["Reviewer Union"])
    ...
    xgtree["classifier"] = classifier = xgb.XGBClassifier(objective="binary:logistic",
    ...
    classifier.fit(training_set.to_numpy(),
                   review_train["Reviewer Union"],
                   verbose=False,
                   eval_set=[(testing_set.to_numpy(), review_test["Reviewer Union"])])

    review_all_decision = classifier.predict_proba(all_set)[:, 1]
    review_test_decision = classifier.predict_proba(testing_set)[:, 1]

    xgtree["all_fpr"], xgtree["all_tpr"], xgtree["all_roc_thres"] = metrics.roc_curve(
        review_dataframe["Reviewer Union"],
        review_all_decision,
    )
    xgtree["test_fpr"], xgtree["test_tpr"], xgtree["test_roc_thres"] = metrics.roc_curve(
        review_test["Reviewer Union"],
        review_test_decision,
    )
    xgtree["all_roc_auc"] = metrics.auc(xgtree["all_fpr"], xgtree["all_tpr"])
    xgtree["test_roc_auc"] = metrics.auc(xgtree["test_fpr"], xgtree["test_tpr"], )

    xgtree["all_pr"], xgtree["all_rec"], thresholds = metrics.precision_recall_curve(
        review_dataframe["Reviewer Union"],
        review_all_decision,
    )
    xgtree["all_prr_thres"] = thresholds

    xgtree["test_pr"], xgtree["test_rec"], xgtree["test_prr_thres"] = metrics.precision_recall_curve(
        review_test["Reviewer Union"],
        review_test_decision,
    )
    xgtree["all_prc_auc"] = metrics.auc(xgtree["all_rec"], xgtree["all_pr"])
    xgtree["test_prc_auc"] = metrics.auc(xgtree["test_rec"], xgtree["test_pr"])
    xgtree["all_prc_f1"] = 2 * (xgtree["all_pr"] * xgtree["all_rec"]) / (xgtree["all_pr"] + xgtree["all_rec"])
    xgtree["test_prc_f1"] = 2 * (xgtree["all_pr"] * xgtree["all_rec"]) / (xgtree["all_pr"] + xgtree["all_rec"])

    p_index_p = int(np.searchsorted(xgtree["all_pr"][1:], target_precision, side="right") - 1)
    ...
    xgtree["false_positives"] = np.where(errors)[0][predicted_class[errors]]
    xgtree["false_negatives"] = np.where(errors)[0][(review_dataframe["Reviewer Union"] == 1)[errors]]
...
if mp:
    with Pool(processes=50) as pool:
        ...
        xgtrees = [w.get(timeout=3600) for w in workers]
else:
    xgtrees = [
        create_xgboost(metrics_dataframe, review_dataframe, metrics_all_scaled, random.randrange(100000000)) for t_set in range(shuffles)
    ]

all_false_positives = np.concatenate([xgtree["false_positives"] for xgtree in xgtrees])
all_false_negatives = np.concatenate([xgtree["false_negatives"] for xgtree in xgtrees])
...
for index, id in enumerate(fp_df["id"]):
    fp_metrics[index] = {"data": bin_data["data"][id], "spectra": bin_data["spectra"][id]}
    fp_labels[index] = bin_data["labels"][id]

fn_metrics = [None] * len(fn_df)
fn_labels = [""] * len(fn_df)
for index, id in enumerate(fn_df["id"]):
    fn_metrics[index] = {"data": bin_data["data"][id], "spectra": bin_data["spectra"][id]}
    fn_labels[index] = bin_data["labels"][id]

# %%
all_fpr = [xgtree["all_fpr"] for xgtree in xgtrees]
all_tpr = [xgtree["all_tpr"] for xgtree in xgtrees]
all_roc_thres = [xgtree["all_roc_thres"] for xgtree in xgtrees]
all_rec = [xgtree["all_rec"] for xgtree in xgtrees]
all_pr = [xgtree["all_pr"] for xgtree in xgtrees]
test_roc_thres = [xgtree["test_roc_thres"] for xgtree in xgtrees]

test_fpr = [xgtree["test_fpr"] for xgtree in xgtrees]
test_tpr = [xgtree["test_tpr"] for xgtree in xgtrees]
all_prr_thres = [xgtree["all_prr_thres"] for xgtree in xgtrees]
test_rec = [xgtree["test_rec"] for xgtree in xgtrees]
test_pr = [xgtree["test_pr"] for xgtree in xgtrees]
test_prr_thres = [xgtree["test_prr_thres"] for xgtree in xgtrees]

#%%
# Plot Performance
importlib.reload(np.core.numeric)  # Pandas causes numpy to break which is dumb....
ts_group = ClassifierPerformanceGroup(layout_settings=dict(title=dict(text=f"{PLOT_NAME} Performance")))
ts_group["all"]["roc"].build(x=all_fpr[0:100], y=all_tpr[0:100], thresholds=all_roc_thres[0:100])
ts_group["all"]["precisionrecall"].build(x=all_rec[0:100], y=all_pr[0:100], thresholds=all_prr_thres[0:100])
ts_group["test"]["roc"].build(x=test_fpr[0:100], y=test_tpr[0:100], thresholds=test_roc_thres[0:100])
ts_group["test"]["precisionrecall"].build(x=test_rec[0:100], y=test_pr[0:100], thresholds=test_prr_thres[0:100])
ts_group.figure.show()
ts_group.figure.write_html(OUT_DIR.joinpath(f"{PLOT_NAME}_Performance_Plot.html").as_posix())
ts_group.figure.update_layout(autosize=False, width=1080, height=1080)
ts_group.figure.write_image(OUT_DIR.joinpath(f"{PLOT_NAME}_Performance_Plot.jpeg").as_posix())
...
if fp_metrics:
    ...
    fp_group["spectra"].build(x=bin_data["frequencies"][0], y=fp_spectra, labels=fp_labels)
...
if fn_metrics:
    ...
    fn_sp_plot = SpectraPlot(subplot=fn_sub[0][2], x=bin_data["frequencies"][0], y=fn_spectra, labels=fn_labels)
