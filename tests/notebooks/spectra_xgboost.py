...
PLOT_NAME = "Normalized_Spectra_Log10_Raw_Power_XGBoost"
TIME_AXIS = 0
CHANNEL_AXIS = 1
LOWER_FREQUENCY = 0
UPPER_FREQUENCY = 500
...
for bin_c in flat_data["data"]:
    ...
    mn = np.nanmean(spectra)
    md = np.nanmedian(spectra)
    sd = np.nanstd(spectra)

    tan_space = np.tan((spectra - 0.5)*np.pi)
    tan_center = np.tan((mn - 0.5)*np.pi)
    tan_scale = np.abs(np.abs(tan_center) - np.abs(np.tan((np.abs(mn-0.5) - 3*sd)*np.pi)))
    tan_mn = np.nanmean(tan_space)
    tan_sd = np.nanstd(tan_space)

    # spectra = np.arctan((tan_space-tan_center)/tan_scale)/np.pi + 0.5
    ...
    flat_data["power"].append(power)
    flat_data["spectra"].append(spectra)
    flat_data["frequencies"].append(freqs)

    # power = np.log10(power)

    ag_metrics.append(np.concatenate([spectra, np.expand_dims(power, axis=0)]))
...
if scale_:
    ...
    metrics_all_scaled = metrics_dataframe
...
def create_xgboost(metrics_df, review_df, mas, rs):
    ...
    if scale_:
        ...
        metrics_train_scaled = metrics_train
        metrics_test_scaled = metrics_test.to_numpy()
    ...
    param_grid = dict(
        max_depth=[1],
        learning_rate=[0.075, 0.1, 0.2, 0.5],
        gamma=[1.2],
        reg_lambda=[1.0],
    ...
    # op = sklearn.model_selection.GridSearchCV(
    ...
    classifier.fit(training_set.to_numpy(),
                   review_train["Reviewer Union"],
                   verbose=False,
                   eval_set=[(testing_set, review_test["Reviewer Union"])])
