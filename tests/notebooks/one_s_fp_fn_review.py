...
ag_metrics = {m: deque() for m in METRICS}
...
for bin_c in bin_data["data"]:
    ...
    r_val = np.corrcoef(spectra, c_data)
    r_squared = r_val[0][1] ** 2
    mae, mse, rmse = calculate_mean_errors(spectra, c_data)

    curve_removed = spectra - c_data
    curve_2 = curve_removed ** 2

    prob = curve_2 / np.sum(curve_2)
    entro = entropy(prob)
    normal_entropy = entro / np.log(prob.shape[0])

    ag_metrics["r_squared"].append(r_squared)
    ag_metrics["normal_entropy"].append(normal_entropy)
    ag_metrics["mae"].append(mae)
    ag_metrics["mse"].append(mse)
    ag_metrics["rmse"].append(rmse)
    ag_metrics["curve_offset"].append(fo.aperiodic_params_[0])
    ag_metrics["curve_exp"].append(fo.aperiodic_params_[1])
    if len(fo.peak_params_) > 0:
        ag_metrics["peak_freq"].append(fo.peak_params_[0][0])
        ag_metrics["peak_height"].append(fo.peak_params_[0][1])
        ag_metrics["peak_width"].append(fo.peak_params_[0][2])
    else:
        ag_metrics["peak_freq"].append(0)
        ag_metrics["peak_height"].append(0)
        ag_metrics["peak_width"].append(0)
...
ts_group["all"]["roc"].build(x=all_fpr[0:50], y=all_tpr[0:50], thresholds=all_roc_thres[0:50])
ts_group["all"]["precisionrecall"].build(x=all_rec[0:50], y=all_pr[0:50], thresholds=all_prr_thres[0:50])
ts_group["test"]["roc"].build(x=test_fpr[0:50], y=test_tpr[0:50], thresholds=test_roc_thres[0:50])
ts_group["test"]["precisionrecall"].build(x=test_rec[0:50], y=test_pr[0:50], thresholds=test_prr_thres[0:50])
